...
- `project_assignments` — links a `client`-role user to the specific project(s) they may see. A `client` is scoped to their own organisation *and* their active assignments — not the whole organisation. Enforced both in the application layer (`backend/app/services/scoping.py`) and, as of `20260713100000_client_project_assignment_rls.sql`, at the RLS layer via `public.client_has_project_access(project_id)`. See `DEVELOPMENT_PLAN.md` Workstream A for why this was added after the fact.
...
- `client_csat_scores` — monthly per-project client satisfaction scores 
...
- `notifications` — in-app alert delivery records sent to users; outbound email delivery is Phase 2+
- `audit_logs` — append-only, general-purpose audit trail for domain events and platform state changes (delivery risk-alert/recommendation changes, governance dashboard exports, etc.); see table definition below for its RLS history

**MVP scope:** Phase 1 includes only Delivery Performance Agent, Quality Intelligence Agent, and Client Interaction Agent. Workforce & Capability, Project Governance, Operational Knowledge, Go-Live Readiness, Margin Analytics, ROI Analytics, Churn Prediction, and other portfolio intelligence features are Phase 2+ and must not appear in the MVP UI.
...
- One **project** has many **project_assignments** (which `client`-role users may see it); one **user** has many **project_assignments** (which projects they may see, if they are a `client`).
...
- One **audit_log** belongs to one **organisation** and optionally one **project**. No FK to **users** — see the table's own section for why.
...
--                        AND public.client_has_project_access(id) -- see project_assignments below
...
```sql
-- Restricts a 'client' user to specific projects within their organisation.
-- delivery_manager, bsg_leadership, and super_admin are unaffected by this
-- table -- they continue to see every project in their org (or all orgs).
-- Added in 20260623114500_project_assignments.sql; RLS enforcement added
-- later in 20260713100000_client_project_assignment_rls.sql (Workstream A,
-- DEVELOPMENT_PLAN.md) after it was found that only the application layer
-- (backend/app/services/scoping.py) enforced this, not the database.
...
-- Helper used by every client-role RLS policy that needs project scoping:
-- CREATE FUNCTION public.client_has_project_access(p_project_id uuid) RETURNS boolean
--   -- true iff an active, non-deleted project_assignments row exists for
--   -- (p_project_id, public.current_user_id())

-- RLS:
--   'client'           → SELECT only their own assignment rows
--   'delivery_manager' → SELECT/INSERT/UPDATE/DELETE where org_id = their org_id
--   'bsg_leadership'   → SELECT all rows (read-only)
--   'super_admin'      → full access
...
-- RLS: mirror projects table policies, using org_id column, plus
--      public.client_has_project_access(project_id) for the 'client' role.
...
-- RLS: 'client' → SELECT where org_id = their org_id AND
--                 public.client_has_project_access(project_id) (read-only).
...
-- RLS: 'client' → SELECT where org_id = their org_id AND
--                 public.client_has_project_access(project_id).
--      'delivery_manager' → SELECT/INSERT/UPDATE where org_id = their org_id.
...
-- RLS: 'client' → SELECT where org_id = their org_id AND the parent
--                 quality_snapshots.project_id passes
--                 public.client_has_project_access() -- this table has no
--                 project_id of its own, only quality_snapshot_id (read-only;
--                 error taxonomy surfaced in client view).
--      'delivery_manager' → SELECT/INSERT/UPDATE where org_id = their org_id.
--      'bsg_leadership' → SELECT all.
--      'super_admin' → full access.
...
-- RLS: 'client' → SELECT where org_id = their org_id AND
--                 public.client_has_project_access(project_id) (read-only;
--                 clients see their own escalations, only for assigned projects).
...
-- RLS: mirror risk_alerts policies, including
--      public.client_has_project_access(project_id) for the 'client' role.
...
-- RLS: 'client' → SELECT where org_id = their org_id AND status = 'sent' AND
--                 public.client_has_project_access(project_id) (only see
--                 approved+sent comms for their assigned projects).
--      'delivery_manager' → SELECT/INSERT/UPDATE where org_id = their org_id
--                 (comms_dm_all, FOR ALL) — triggers draft generation and can
--                 approve/send. See the resolved-F3 note below.
--      'bsg_leadership' → SELECT all.
--      'super_admin' → full access.
-- NOTE (DEVELOPMENT_PLAN.md Workstream D / F3 — resolved 2026-07-13): an
--   earlier version of this doc said, in bold, that INSERT is only permitted
--   by the backend service account (service_role) and never by
--   'delivery_manager' directly. That was never how this table was built:
--   the RLS policy (comms_dm_all, FOR ALL, not just UPDATE) and the route
--   layer (backend/app/api/routes/communications.py, POST
--   /projects/{id}/communications/draft, gated to delivery_manager/
--   super_admin) have always let 'delivery_manager' and 'super_admin' insert
--   a draft directly through their own request-scoped session. There is no
--   service_role-authenticated write path anywhere in this codebase for this
--   table. Decision: the code is correct, this doc was wrong. "AI-drafted"
--   (see the table's one-line description above) refers to who writes
--   body_draft's *content* (an LLM call in
--   app/services/communications.py:generate_comms_draft_body, labelled by
--   the drafted_by_agent column) — not who performs the database write. The
--   DM/super_admin triggers the HTTP request that both generates the text
--   and inserts the row in one step; no separate agent identity is
--   involved. Covered by backend/tests/test_communications_rbac.py at the
--   route layer and backend/tests/rls/test_role_visibility.py at the RLS
--   layer.
...
```sql
-- NOTE: this section originally specified a denormalized org_id column here
-- "for RLS", per the Standard Fields Convention in Section 5. The table as
-- actually implemented (20260622090000_initial_backend_schema.sql) was
-- never given that column -- and the RLS policies written against it
-- referenced org_id anyway, which means those CREATE POLICY statements
-- could not have succeeded ("column org_id does not exist"). Fixed in
-- 20260713100000_client_project_assignment_rls.sql by rewriting the
-- affected policies to join through communication_id -> client_communications
-- instead. This doc now reflects the table as it actually exists. Adding
-- org_id back as a real denormalized column (with a backfill) would restore
-- consistency with every other table's convention and is a reasonable
-- follow-up, but is not required for correct access control.
...
-- RLS: 'client' → SELECT where the parent client_communications row has
--                 org_id = their org_id AND status = 'sent' AND
--                 public.client_has_project_access(project_id).
--      'delivery_manager' → SELECT where the parent's org_id = their org_id.
--      'bsg_leadership' → SELECT all.
--      'super_admin' → full access.
...
-- RLS: 'client' → SELECT where org_id = their org_id AND user_id =
--                 public.current_user_id() AND (project_id IS NULL OR
--                 public.client_has_project_access(project_id)) -- a client's
--                 own cross-project query (project_id NULL) stays visible;
--                 a project-scoped query only if still assigned to it.
...
```sql
-- NOTE: the FK column here is `agent_query_id` in the actual implementation,
-- not `query_id` as an earlier version of this doc had it -- corrected below.
-- Also, like communication_evidence_links above, this doc previously
-- specified a denormalized org_id column that was never actually added to
-- the table; the RLS policies that referenced it directly could not have
-- succeeded. Fixed in 20260713100000_client_project_assignment_rls.sql by
-- joining through agent_query_id -> agent_queries instead.
...
CREATE INDEX agent_query_evidence_links_query_id_idx ON agent_query_evidence_links (agent_query_id);

-- RLS: 'client' → SELECT where the parent agent_queries row has org_id =
--                 their org_id AND user_id = public.current_user_id() AND
--                 (project_id IS NULL OR public.client_has_project_access(project_id)).
--      'delivery_manager' → SELECT where the parent's org_id = their org_id.
--      'bsg_leadership' → SELECT all.
--      'super_admin' → full access.
...
--   MVP decision: CSAT is collected monthly per project, submitted only by
--   client-role users. Scale is 1-5. reporting_period_month is the first day
--   of the reporting month, stored as date format DD-MM-YYYY.
...
CREATE TABLE client_csat_scores (
  ...
  reporting_period_month DATE  NOT NULL,                                                   -- first day of reporting month, e.g. 01-06-2026
  ...
  CONSTRAINT client_csat_scores_project_user_month_key
    UNIQUE (project_id, submitted_by, reporting_period_month)
);
...
CREATE INDEX client_csat_scores_period_idx ON client_csat_scores (reporting_period_month);
...
CREATE TABLE notifications (
  ...
  sent_at           TIMESTAMPTZ,                       -- NULL until in-app delivery is confirmed; outbound email is Phase 2+
  created_at        TIMESTAMPTZ         NOT NULL DEFAULT now(),
  updated_at        TIMESTAMPTZ         NOT NULL DEFAULT now()
);
...
```sql
-- General-purpose append-only audit trail, added outside the original
-- Phase-1 schema (not present in the initial table list above until this
-- note was added). Written by two call sites today: AuditLogger
-- (backend/app/agents/delivery/audit/audit_logger.py, delivery domain
-- events) and log_governance_event (backend/app/agents/governance/services/audit_service.py,
-- governance actions and dashboard exports). Both write through the
-- caller's own request-scoped, RLS-bound session -- never service_role.
--
-- NOTE: unlike every other audit table in this schema (agent_queries,
-- client_communications), this table has no user_id column, so "who did
-- it" is not independently queryable -- only inferable from payload
-- contents if the caller chose to include it. This does not meet
-- 13. Security & Compliance.md SS11.2's "every audit event must capture
-- user_id" requirement. Flagged as a gap (DEVELOPMENT_PLAN.md), not fixed
-- as part of Workstream B -- it needs a migration, an ORM change, and
-- updates to every call site, none of which is required just to fix the
-- RLS gap below.
--
-- : this table ran ALTER TABLE ... ENABLE ROW LEVEL SECURITY with zero
-- CREATE POLICY statements from 20260624120000_audit_logs.sql until
-- 20260713110000_audit_logs_rls.sql (DEVELOPMENT_PLAN.md Workstream B) --
-- the only such gap found across all 67 RLS-enabled tables in this schema
-- (backend/tests/test_migrations_rls_policy_coverage.py now guards against
-- a recurrence). Every write through a non-superuser connection would have
-- been silently denied.
...
CREATE TABLE audit_logs (
  ...
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
  -- Immutable: append-only, same as agent_queries. No updated_at, no soft delete.
);
...
-- RLS: 'client', 'delivery_manager', 'bsg_leadership', 'super_admin' → INSERT
--                where org_id = their org_id (confirmed call sites span all
--                four roles -- e.g. a client exporting a governance analytics
--                CSV logs an audit event for their own org).
--      'delivery_manager' → SELECT where org_id = their org_id.
--      'bsg_leadership' → SELECT all.
--      'super_admin' → SELECT all.
--      No SELECT policy for 'client' -- no current route exposes audit_logs
--      to client users, matching the audit-export access list in
--      13. Security & Compliance.md SS11.4.
...
8. **CSAT submitter and cadence check:** Inserts into `client_csat_scores` must verify `submitted_by` carries `role = 'client'`, `reporting_period_month` is the first day of a month, and no existing row already exists for the same `(project_id, submitted_by, reporting_period_month)`.

9. **Communication draft ownership (resolved 2026-07-13, DEVELOPMENT_PLAN.md Workstream D / F3):** `delivery_manager` and `super_admin` insert `client_communications` rows directly, via `POST /projects/{id}/communications/draft` (`backend/app/api/routes/communications.py`), writing through their own RLS-scoped session under the `comms_dm_all` policy. There is no `service_role`-only write path for this table. A prior version of this rule said only `service_role` may INSERT and DMs may only UPDATE — that was never how the table was implemented, and the decision (see `10. DB Schema.md` §7's `client_communications` note) was that the code, not the doc, was correct.
...
users               [1] ─── [N] project_assignments  (client-role scoping)
...
except communication_evidence_links and agent_query_evidence_links, which
reach org_id by joining through their parent row instead (see those tables'
...
| 5 | `risk_alerts.contributing_causes` | `JSONB` is used for the contributor breakdown. Confirm this is acceptable or whether a separate `risk_alert_causes` child table is preferred for queryability. |
| 6 | `metric_configurations` | Is this truly platform-wide (one global config) or per-organisation (each client can have a different metric set)? Source says Super Admin configures it "system-wide" — assumed global here. |
| 7 | `quality_snapshots` granularity | Source shows data "by team and by week". Confirm whether a finer granularity (daily, per annotator) is needed for Phase 1, or weekly per team is sufficient for MVP. |
| 8 | `teams.domain` | Is `domain` a free-text field or a constrained enum? Source names: Radiology CV, Clinical NLP, Pathology, Genomics, General CV. A new client vertical (Finance, Logistics) would add different domain names. |
| 9 | Workforce utilisation history | Workforce utilisation history is Phase 2+ and not modelled for MVP. Confirm later whether it should be derived or stored as a time-series table. |
| 10 | `updated_at` trigger | Confirm whether `updated_at` is maintained by a PostgreSQL `BEFORE UPDATE` trigger (preferred for reliability) or by the application layer (SQLAlchemy `onupdate`). |
| 11 | `project_assignments` scope | Assumed (not yet formally confirmed by product/security) that project-assignment scoping applies to every `client` user unconditionally, with no per-organisation opt-out — matching the existing, unconditional behaviour of `backend/app/services/scoping.py`. See `DEVELOPMENT_PLAN.md` Workstream A step 1. |
| 12 | `communication_evidence_links` / `agent_query_evidence_links` org_id | These two tables lack the denormalized `org_id` column every other client-scoped table has, and their RLS policies now join through the parent row instead (see those tables' sections). Confirm whether to leave this as the permanent design, or do a follow-up migration to add `org_id` back for consistency with the rest of the schema. Not urgent — access control is correct either way. |

---

## 10. Final Resolved Decisions

| Decision | Resolution |
|---|---|
| Date format | All database `DATE` values use `DD-MM-YYYY` in examples and API payloads. Timestamps are timezone-aware `TIMESTAMPTZ`. |
...
| CSAT uniqueness | `client_csat_scores` enforces one row per `(project_id, submitted_by, reporting_period_month)`. |
| Report generation | Weekly Client Summaries and Executive Reports are manually generated in MVP. No report scheduler tables are required in Phase 1. |
| Report export/delivery | MVP reporting is in-app only. PDF export, DOCX export, and outbound email delivery are Phase 2+. |
| Notifications | MVP notifications are stored for in-app delivery. Email notification delivery is Phase 2+. |
| Go-Live Readiness | Go-Live Readiness is Phase 2+ and has no MVP table. |
| Workforce imbalance UI | Workforce & Capability features are Phase 2+ and must not appear in the MVP UI, even if enum compatibility values exist. |
| Client project scoping | `client` users are scoped to their organisation *and* their active `project_assignments` rows, not the whole organisation. This is enforced in the application layer (`backend/app/services/scoping.py`) and, as of `20260713100000_client_project_assignment_rls.sql`, at the RLS layer via `public.client_has_project_access(project_id)`. |
