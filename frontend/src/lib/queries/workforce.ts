...
export function projectWorkforceSummaryQueryOptions(projectId: string | null, enabled = true) {
  return queryOptions({
...
export function buildProjectWorkforceSummary(
  ...
  const smeCoveragePct =
    activeAnnotators.length > 0 ? Math.round((smeCount / activeAnnotators.length) * 100) : null;
...
function buildAnnotatorsByTeamFromList(
  ...
  for (const annotator of annotators) {
    const list = map.get(annotator.team_id);
    if (list) {
      list.push(annotator);
    }
  }
...
export function useProjectWorkforceSummary(projectId: string | null, canReadAnnotators: boolean) {
  const teamsQuery = useProjectTeamsQuery(canReadAnnotators ? null : projectId);
  const summaryQuery = useQuery(projectWorkforceSummaryQueryOptions(projectId, canReadAnnotators));

  const teams = useMemo(
    () => (canReadAnnotators ? (summaryQuery.data?.teams ?? []) : (teamsQuery.data ?? [])),
    [canReadAnnotators, summaryQuery.data, teamsQuery.data],
  );

  const annotatorsByTeam = useMemo(() => {
    if (!canReadAnnotators) {
      return new Map<string, AnnotatorRead[]>();
    }
    const annotators = summaryQuery.data?.annotators ?? [];
    return buildAnnotatorsByTeamFromList(teams, annotators);
  }, [canReadAnnotators, summaryQuery.data, teams]);
  ...
  const activeQuery = canReadAnnotators ? summaryQuery : teamsQuery;

  return {
    teamsQuery: canReadAnnotators
      ? { ...summaryQuery, data: summaryQuery.data?.teams }
      : teamsQuery,
    annotatorQueries: [],
    summary,
