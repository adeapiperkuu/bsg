...
function DeliveryMarkdownComponent({ content, className }: Props) {
  ...
  const blocks = useMemo(() => {
    ...
    const flushTable = () => {
      ...
      renderedBlocks.push(
        <div
          key={`table-${renderedBlocks.length}`}
    ...
    for (const line of lines) {
      ...
      if (trimmed === "---" || trimmed === "***") {
        ...
        flushTable();
        renderedBlocks.push(
          <hr key={`hr-${renderedBlocks.length}`} className="my-3 border-border/50" />,
        );
        continue;
      }
      ...
      flushList();
      renderedBlocks.push(
        <p key={`p-${renderedBlocks.length}`} className="text-[11px] leading-5 text-foreground">
