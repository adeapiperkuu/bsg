...
const reports = [
  {
    name: "Aurora Health — Weekly Status W24",
    state: "Approved",
    date: "Jun 17",
    client: "Aurora Health",
  },
  {
    name: "Helios Bank — Schema Progress",
    state: "Pending",
    date: "Jun 18",
    client: "Helios Bank",
  },
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <Card className="lg:col-span-1">
        ...
        <div className="mb-3 flex gap-1 text-[11px]">
          {["All", "Draft", "Pending", "Approved"].map((s) => (
            <button key={s} className="rounded border border-border bg-elevated px-2 py-0.5">
              {s}
            </button>
          ))}
        </div>
        <ul className="space-y-1.5 text-xs">
          {reports.map((r) => (
            <li key={r.name}>
              <button
                onClick={() => setSel(r)}
                className={`flex w-full items-center justify-between rounded border border-border px-2.5 py-2 text-left ${sel.name === r.name ? "bg-elevated" : ""}`}
              >
      ...
      <Card className="lg:col-span-2">
        <SectionHeader
          title={sel.name}
          sub={`${sel.client} · ${sel.date}`}
          right={
            <div className="flex gap-2">
              <AiBadge confidence={91} />
              <button className="rounded border border-[color:var(--info)]/30 bg-[color:var(--info)]/15 px-2.5 py-1 text-[11px] font-medium text-[color:var(--info)]">
                Edit
              </button>
        ...
        <div className="prose-invert max-w-none space-y-3 rounded-md border border-border bg-elevated p-5 text-sm leading-6">
          ...
          <p>
            This week {sel.client} maintained healthy operational momentum across all active
            engagements. Schedule confidence improved by 1.4 points over the prior week, and
            gold-set accuracy held at 94.2%.
          </p>
          <h3 className="text-base font-semibold">Delivery Highlights</h3>
          <ul className="ml-5 list-disc">
            <li>Batch 14 completed 2 days ahead of schedule</li>
            <li>3 new SMEs onboarded to the radiology pod</li>
            <li>Schema v2 review underway with client team</li>
          </ul>
          <h3 className="text-base font-semibold">Risks & Mitigations</h3>
          <p>
            One drift signal flagged in the radiology subset; calibration session scheduled for Jun
            22. No client-facing impact expected.
          </p>
          ...
          >
            ...
            <div className="mt-3 space-y-3 text-xs">
              <label className="block">
                ...
                <select className="mt-1 w-full rounded border border-border bg-elevated px-2 py-1.5">
                  {reports.map((r) => (
                    <option key={r.client}>{r.client}</option>
                  ))}
                </select>
              </label>
              <label className="block">
                ...
                <select className="mt-1 w-full rounded border border-border bg-elevated px-2 py-1.5">
                  ...
                  <option>Quality Note</option>
                  <option>Capacity Forecast</option>
                </select>
              </label>
              <label className="block">
