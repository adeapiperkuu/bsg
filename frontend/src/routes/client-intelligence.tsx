    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Card>
          ...
          <div className="mt-2 h-2 overflow-hidden rounded bg-elevated">
            <div className="h-full bg-[color:var(--brand)]" style={{ width: "75%" }} />
          </div>
        </Card>
          ...
          <div className="mt-2 flex gap-0.5">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                ...
                className={
                  s <= 4
                    ? "h-5 w-5 fill-[color:var(--warning)] text-[color:var(--warning)]"
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {clients.map((c) => (
                  <tr
                    key={c.name}
                    className={`cursor-pointer border-b border-border/50 hover:bg-elevated ${sel === c.name ? "bg-elevated" : ""}`}
        ...
        <Card className="lg:col-span-2">
          <SectionHeader
            title={selected ? `${selected.name} · Detail` : "Client Detail"}
            sub={selected ? "Narrative · drafts · Q&A" : "Select a client row"}
            right={selected ? <AiBadge confidence={89} /> : null}
          />
          {!selected && (
            <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
              Pick any client from the table to view AI-curated narrative, draft reports queue, and
              Q&A log.
            </div>
          )}
          {selected && (
            <div className="space-y-4">
              <div className="rounded-md border border-border bg-elevated p-3 text-xs leading-5">
                ...
                {selected.name} is currently tracking at {selected.confidence}% schedule confidence
                across {selected.projects} active project{selected.projects > 1 ? "s" : ""}.{" "}
                {selected.health === "Critical"
                  ? "Immediate intervention required — see escalations."
                  : selected.health === "At Risk"
                    ? "Mitigation in progress; expected recovery within 2 weeks."
                    : "All milestones aligned to plan."}
              </div>
              <div>
                ...
                <ul className="space-y-1.5 text-xs">
                  {["Weekly Status W24", "Mid-month Quality Note", "Capacity Forecast Q3"].map(
                    (r) => (
                      <li
                        key={r}
                      ...
                      >
                        <span>{r}</span>
                        <div className="flex gap-1">
                          <button className="rounded border border-border px-1.5 py-0.5 text-[10px]">
                            Preview
                          </button>
                          <button className="rounded border border-border px-1.5 py-0.5 text-[10px]">
                            Edit
                          </button>
                          <button className="rounded bg-[color:var(--brand)] px-1.5 py-0.5 text-[10px] text-[color:var(--brand-foreground)]">
                            Approve
                          </button>
                          <button className="rounded border border-border px-1.5 py-0.5 text-[10px]">
                            Reject
              ...
              <div>
                ...
                <ul className="space-y-2 text-xs">
                  <li className="rounded border border-border bg-elevated p-2">
                    <div className="font-medium">
                      Q: What's the projected delivery date for batch 14?
                    </div>
                    <div className="mt-1 text-muted-foreground">
                      A (AI draft): Estimated Jun 24, 94% confidence.{" "}
                      <span className="ml-1 text-[10px] text-[color:var(--brand)]">Approve →</span>
                    </div>
                  </li>
                  <li className="rounded border border-border bg-elevated p-2">
                    <div className="font-medium">Q: Can we expand to a 4th annotator pod?</div>
                    <div className="mt-1 text-muted-foreground">
                      A (AI draft): Capacity available in Kosovo from Jul 8. Pending PM review.
                    </div>
                  </li>
                </ul>
              </div>
