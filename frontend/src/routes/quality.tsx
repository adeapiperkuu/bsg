...
function QualityPage() {
  return (
    <div className="space-y-5">
      ...
      <Card>
        <SectionHeader title="Quality Trend" sub="Gold accuracy & IAA · 12 weeks" right={<div className="flex gap-1.5"><button className="rounded border border-border px-2 py-0.5 text-[11px]">All teams</button><button className="rounded border border-border bg-elevated px-2 py-0.5 text-[11px]">Radiology</button></div>} />
        ...
        <Card>
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
            {errorCategories.map((e) => (
              <span key={e.cat} className="rounded border border-border bg-elevated px-2 py-0.5">{e.cat} <span className={e.delta.startsWith("+") ? "text-[color:var(--danger)]" : "text-[color:var(--success)]"}>{e.delta}</span></span>
            ))}
          </div>
        </Card>
        ...
        <Card>
          ...
          <ul className="space-y-2">
            <li className="rounded-md border border-border bg-elevated p-3 text-xs">
              <div className="flex items-center gap-2"><StatusPill status="Critical" /> <span className="font-medium">Pulse Medical · Radiology subset</span></div>
              <div className="mt-1 text-muted-foreground">IAA dropped to 0.81 (threshold 0.85). Recommended: schedule reviewer calibration session.</div>
              <button className="mt-2 rounded bg-[color:var(--brand)] px-2.5 py-1 text-[11px] font-medium text-[color:var(--brand-foreground)]">Schedule calibration</button>
            </li>
            <li className="rounded-md border border-border bg-elevated p-3 text-xs">
              <div className="flex items-center gap-2"><StatusPill status="Warning" /> <span className="font-medium">Nimbus NLP · Guideline v3</span></div>
              <div className="mt-1 text-muted-foreground">Class confusion up 8% — guideline ambiguity suspected. Recommended: workshop with SMEs.</div>
              <button className="mt-2 rounded bg-[color:var(--brand)] px-2.5 py-1 text-[11px] font-medium text-[color:var(--brand-foreground)]">Open workshop draft</button>
            </li>
          </ul>
        </Card>
      </div>
      ...
      <Card>
        <SectionHeader title="AI Quality Narrative" sub="Auto-generated · Week 24" right={<div className="flex gap-2"><AiBadge confidence={90} /><EvidenceBadge /></div>} />
        <p className="text-sm leading-6 text-foreground/90">
          Overall quality posture remains strong with gold-set accuracy at 94.2% across the portfolio. The Radiology Pod B team has shown a sustained 3-week IAA decline, now sitting at 0.81 — below the 0.85 operating threshold. Root cause analysis indicates boundary precision errors on lesion contouring, correlated with the introduction of the new pediatric subset in W22. NLP Curation drift appears guideline-driven; a v3 calibration workshop is recommended within 5 business days.
        </p>
        <div className="mt-2 flex flex-wrap gap-1.5">
          <span className="rounded border border-border bg-elevated px-2 py-0.5 text-[10px]">📎 iaa-pod-b-w22.csv</span>
          <span className="rounded border border-border bg-elevated px-2 py-0.5 text-[10px]">📎 boundary-errors.json</span>
          <span className="rounded border border-border bg-elevated px-2 py-0.5 text-[10px]">📎 guideline-v3-diff.md</span>
        </div>
      </Card>
    </div>
  );
