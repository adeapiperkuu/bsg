    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
                ...
                <tbody>
                  {dependencies.map((d) => (
                    <tr key={d.name} className="border-b border-border/50">
                      ...
                      <td className="py-2.5 pr-3">{d.overdue > 0 ? <span className="text-[color:var(--danger)]">{d.overdue}d</span> : "—"}</td>
                      <td className="py-2.5 pr-3"><StatusPill status={d.status} /></td>
                      <td className="py-2.5 pr-3"><button className="rounded border border-border px-2 py-0.5 text-[10px]">Mark Resolved</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card>
            <SectionHeader title="Governance Register" sub="Project · scope · dependencies · escalations" />
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="text-left text-muted-foreground"><tr className="border-b border-border">
                ...
                <tbody>
                  {[
                    ...
                    { p: "Helios Docs", scope: "Pending v2", deps: "1 blocking", actions: 5, esc: 1 },
                    ...
                    <tr key={r.p} className="border-b border-border/50">
                      ...
                      <td className="py-2.5 pr-3"><StatusPill status={r.scope === "Approved" ? "On Track" : r.scope === "Pending v2" ? "At Risk" : "In Progress"} /></td>
                      <td className="py-2.5 pr-3 text-muted-foreground">{r.deps}</td>
                      <td className="py-2.5 pr-3">{r.actions}</td>
                      <td className="py-2.5 pr-3">{r.esc > 0 ? <span className="text-[color:var(--danger)]">{r.esc}</span> : "0"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Card>
          ...
          <div className="rounded-md border border-[color:var(--brand)]/30 bg-[color:var(--brand)]/5 p-3 text-xs">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Next governance call</div>
          ...
          <div className="mt-4">
            <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Due this week</div>
            <ul className="space-y-1.5 text-xs">
              {[
                ...
                <li key={i.t} className="flex items-center justify-between rounded border border-border bg-elevated px-2.5 py-1.5">
                  <span className="flex items-center gap-2"><input type="checkbox" className="accent-[color:var(--brand)]" /><span>{i.t}</span></span>
          ...
          <div className="mt-4 rounded-md border border-border bg-elevated p-3 text-xs">
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Auto-generated summary</div>
            <p className="leading-5 text-foreground/90">
              4 governance items pending; Helios schema is the critical-path blocker. Nimbus capacity proposal ready for review. Two escalations require client input on Friday's call.
            </p>
            <button className="mt-2 rounded bg-[color:var(--brand)] px-2.5 py-1 text-[11px] font-medium text-[color:var(--brand-foreground)]">Approve Summary</button>
          </div>
        </Card>
      </div>
            ...
            <tbody>
              {escalations.map((e) => (
                <tr key={e.title} className="border-b border-border/50">
                  ...
                  <td className="py-2.5 pr-3"><StatusPill status={e.severity} /></td>
