...
function Ask() {
  ...
  const [m, setM] = useState<Msg[]>([
    {
      role: "ai",
      text: "Hi — I can answer questions about your projects, milestones and reports. Anything outside that scope will be routed to your BSG PM.",
    },
  ]);
  ...
  const send = () => {
    ...
    setM((p) => [
      ...
      {
        role: "ai",
        text: "Based on this week's data, your batch 14 is on track for Jun 24 delivery with 94% confidence.",
      },
    ]);
    setV("");
  };
  return (
    <Card>
      <SectionHeader
        title="Ask Agent"
        sub="Restricted to your projects"
        right={<AiBadge confidence={91} />}
      />
      <div className="mb-3 max-h-[420px] space-y-2 overflow-y-auto">
        {m.map((x, i) => (
          <div
            ...
            className={
              x.role === "ai"
                ? "rounded-md border border-border bg-elevated p-3 text-xs"
                : "rounded-md bg-[color:var(--brand)]/10 p-3 text-xs"
            }
          >
            <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {x.role === "ai" ? "BSG Agent" : "You"}
            </div>
            <div>{x.text}</div>
          </div>
        ))}
