...
const trend = Array.from({ length: 12 }, (_, i) => ({ week: `W${i + 13}`, confidence: 86 + Math.round(Math.sin(i / 2) * 4) }));
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <table className="w-full text-xs">
          <thead className="text-left text-muted-foreground"><tr className="border-b border-border"><th className="py-2 pr-3 font-medium">Milestone</th><th className="py-2 pr-3 font-medium">Due</th><th className="py-2 pr-3 font-medium">Confidence</th><th className="py-2 pr-3 font-medium">Status</th></tr></thead>
          <tbody>
            {[
              ...
              <tr key={r[0] as string} className="border-b border-border/50"><td className="py-2.5 pr-3 font-medium">{r[0]}</td><td className="py-2.5 pr-3">{r[1]}</td><td className="py-2.5 pr-3">{r[2]}%</td><td className="py-2.5 pr-3"><StatusPill status={r[3] as string} /></td></tr>
            ))}
          </tbody>
        </table>
      </Card>
