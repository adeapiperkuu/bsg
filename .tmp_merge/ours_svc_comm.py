...
def build_fallback_body(
    ...
    *,
    ...
    Produces structured Markdown suitable for DeliveryMarkdown preview ΓÇö not a
    ...
    if milestones:
        ...
        for milestone in milestones[:3]:
            ...
            sections.append(f"- **{name}** ΓÇö {status_label}")
...
async def create_communication_draft(
    ...
    *,
    ...
    """End-to-end synchronous draft: evidence ΓåÆ LLM/fallback ΓåÆ persist. Never auto-approves/sends."""
...
async def move_to_review(
    ...
    """Submit edited content for review (draft ΓåÆ in_review) or re-save while in_review.

    Writes the working body into `body_approved` and sets status to `in_review`.
    This is not a silent draft-only save ΓÇö use `update_communication_content` for that.
    """
    ...
    *,
    ...
    if body is not None:
        ...
        if communication.status == CommunicationStatus.DRAFT:
            ...
            # in_review ΓÇö edit the working approved body shown to reviewers
